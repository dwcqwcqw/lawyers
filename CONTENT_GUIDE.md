...
配置集中于 `lib/site.ts`。当前 `origin` 为私有结构预览地址，`indexable: false`，robots 禁止抓取、页面带 noindex，避免草稿站进入索引。正式公开时必须同时完成：
...
本站为可继续接入内容的结构版本。私有预览无法参与四平台自然检索；技术可读取和实际被AI引用/推荐需分别验证。
...
Vercel 生产地址可公开访问；当前 indexable=false 保留了预览阶段的 noindex，需要正式开放搜索时再切换并重新导出部署。不要继续上传 Sites。
